...
export default function CanvasExtensionsScreen() {
  ...
  const isBusy =
    setEnabled.isPending || refresh.isPending || uninstall.isPending;
  ...
  const confirmationText =
    pendingAction?.type === "enable"
      ? `${t(I18nKey.SETTINGS$CANVAS_EXTENSIONS_TRUST_NOTICE)} Enable ${pendingAction.extension.manifest.display_name || pendingAction.extension.name}?`
      : pendingAction?.type === "uninstall"
        ? `Uninstall ${pendingAction.extension.manifest.display_name || pendingAction.extension.name} from this backend?`
        : "";
              ...
              <div className={extensionModuleCardGridClassName}>
                {query.data.map((extension) => (
                  <CanvasExtensionCard
                    ...
                    onRefresh={() => refresh.mutate(extension.name)}
                    onUninstall={() =>
