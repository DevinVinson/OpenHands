...
export const CANVAS_EXTENSIONS_HANDLERS = [
  http.get("*/api/canvas-extensions/installed", () =>
    HttpResponse.json({
      extensions: installedExtension ? [installedExtension] : [],
    }),
  ),
  ...
  http.post("*/api/canvas-extensions/install", async ({ request }) => {
    ...
    return HttpResponse.json(installedExtension, { status: 201 });
  }),
  ...
  http.post("*/api/canvas-extensions/installed/:name/refresh", ({ params }) => {
    const name = getRequestedName(params.name);
    ...
    persistInstalledExtension({
      ...extension,
      resolved_ref: "mock-working-tree-refreshed",
      installed_at: new Date().toISOString(),
    });
    return HttpResponse.json({
      message: "Canvas extension refreshed.",
