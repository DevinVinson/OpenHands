...
- **Re-enable:** turn it on again. The item and page should return without a
  Canvas restart.
- **Refresh:** select **Refresh**. The displayed resolved ref should change from
  `mock-working-tree` to `mock-working-tree-refreshed`, while enablement remains
  unchanged.
...
- immutable revision resolution and atomic refresh;
